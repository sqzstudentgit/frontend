...
class ImageViewer extends React.Component {
    ...
    render() {
        if(this.props.imageList==null || this.props.imageList.length==0) 
        {
            return (
        ...
        else
        {
            try
            {
                ...
                const tab3DList = this.props.imageList.filter((imageInfo)=>imageInfo.is3DModelType == "Y" ).map((imageInfo) => {
                    ...
                    ?
                        <TabPane  key={"3D"+imageInfo.id.toString()} tab={<Image preview={false} width="50" height="50" style = {{borderRadius: "10%"}} src={modelIcon} alt="3D Model"/>} >
                            <div style={{height:this.props.height*0.75, width:'100%'}} >
                                <div>
                ...
                const tabImageList = this.props.imageList.filter((imageInfo)=>imageInfo.is3DModelType == "N" ).map((imageInfo) => {
                    ...
                    ?
                        <TabPane key={imageInfo.id.toString()} tab={<Image preview={false} width="50" height="50" style = {{borderRadius: "10%"}}  src={imageInfo.smallImageLocation} fallback ={fallbackSrc}/>} >
                            <Image 
                                width={"100%"}
                                style = {{borderRadius: "5%"}}  
                                src = {imageInfo.largeImageLocation} 
